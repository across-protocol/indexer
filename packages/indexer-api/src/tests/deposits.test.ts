...
import { updateDeposits } from "@repo/indexer";
...
describe("Deposits Service Tests", () => {
  ...
  it("should return DepositForBurn deposits with CCTP events", async () => {
    ...
    let savedEvent = await depositForBurnRepo.save(
      depositForBurnRepo.create({
        burnToken: "0x123",
        amount: "1000000",
        depositor: "0xdepositor",
    ...
    // Create MessageSent
    const messageSent = await messageSentRepo.save({
      chainId,
      blockNumber: 1000,
      transactionHash: txHash,
      transactionIndex: 0,
    ...
    // Update deposits with DepositForBurn and MessageSent
    await updateDeposits({
      dataSource: dataSource,
      depositUpdate: {
        cctp: {
          deposit: {
            depositForBurn: savedEvent,
