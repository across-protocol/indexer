...
export class CCTPRepository extends dbUtils.BlockchainEventRepository {
  ...
  public async formatAndSaveBurnEvents(
    ...
    const chunkedEvents: BurnEventsPair[][] = across.utils.chunk(
      burnEvents,
      this.chunkSize,
    );
  ...
  public async formatAndSaveArbitraryActionsExecutedEvents(
    ...
    const chunkedEvents: Partial<entities.ArbitraryActionsExecuted>[][] =
  ...
  public async formatAndSaveSponsoredBurnEvents(
    ...
    const chunkedEvents: Partial<entities.SponsoredDepositForBurn>[][] =
      across.utils.chunk(formattedEvents, this.chunkSize);
    const savedEvents = await Promise.all(
      chunkedEvents.map((eventsChunk) =>
  ...
  public async formatAndSaveMintEvents(
    ...
    const chunkedEvents: MintEventsPair[][] = across.utils.chunk(
      mintEvents,
      this.chunkSize,
    );
    for (const eventsChunk of chunkedEvents) {
