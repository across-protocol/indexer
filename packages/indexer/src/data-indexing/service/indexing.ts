...
> {
  ...
  protocols: SupportedProtocols<TDb, TPayload, TPreprocessed, TRepository, TTransformed, TStoredEventEntity>[];
...
export async function startChainIndexing<
  ...
  TTransformed,
  TStoredEventEntity
>(request: StartIndexerRequest<TDb, TPayload, TPreprocessed, TRepository, TTransformed, TStoredEventEntity>) {
  const {
    database,
    rpcUrl,
    logger,
