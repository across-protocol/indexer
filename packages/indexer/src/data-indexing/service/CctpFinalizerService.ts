...
export class CctpFinalizerServiceManager {
  ...
  private lighterFinalizerService: LighterCctpFinalizerService;
  ...
  public async start() {
    try {
      ...
      this.lighterFinalizerService = new LighterCctpFinalizerService(
        this.logger,
        this.postgres,
        this.pubSubService,
        this.config,
...
/**
 * Temporary class for staging testing
 */
class LighterCctpFinalizerService extends RepeatableTask {
  constructor(
  ...
  protected async taskLogic(): Promise<void> {
    try {
      const qb = this.postgres
        ...
        .andWhere("burnEvent.destinationDomain = 0")
        .andWhere("burnEvent.destinationCaller = :destinationCaller", {
          destinationCaller: "0x5616194d65638086a3191B1fEF436f503ff329eC",
        })
      ...
      this.logger.error({
        at: "LighterCctpFinalizerService#taskLogic",
        message: "Error in LighterCctpFinalizerService",
